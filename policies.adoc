...
** *3.2.3.6.* End users and PIs are allowed only one active or queued print in the space at a time. For instance, someone may not have a print in the queue on a Bambu Labs printer while actively printing on a Formlabs printer.
...
** *4.2.4.2.* End users are welcome to set up cuts themselves, but PIs must verify the settings of every print before it starts. PIs must give the go ahead for every laser cut before an end user presses start.
...
** *4.2.6.4.* If a printer malfunctions or gives an error, PIs should attempt to fix it or contact and MPI or Staff for assistance.
...
* *4.2.8.* Floating PI Responsibilities
** *4.2.8.1.* PIs should spend the first 10 minutes of their shift making rounds around the space to see if any spots need to be filled.
** *4.2.8.2.* PIs should continue to monitor the slack to see if any spaces need to be filled throughout their shift.
** *4.2.8.3.* If not actively filling a spot in another technical area, PIs should be assisting the front desk and benchtop PIs with their duties. They may follow the front desk homework guidelines outlined in section 4.2.2.1.
** *4.2.8.4.* It is acceptable for a PI to sit at the front desk or lectern when not making rounds, but they should not be there exclusively for their entire shift.
** *4.2.8.5.* Despite policy 4.2.8.3, being a floating PI is not an excuse for one to only do homework during their shift. They are still responsible for monitoring the space.
