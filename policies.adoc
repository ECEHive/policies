...
** *3.2.3.3.* There is an 8-hour time limit for all prints in the space. Prints beyond 8 hours can only be approved by a 3D printing MPI or Staff.
...
** *3.2.3.6.* Staff and the 3D printing MPIs reserve the right to deny any model from being printed. Staff and the 3D MPIs reserve the right to cancel any print at any time. The completion of a print in the space is not guaranteed.
** *3.2.3.9.* The Hive is not responsible for prints after they are placed in the pickup area.
** *3.2.3.10.* End users and PIs are allowed only one active or queued print in the space at a time. For instance, someone may not have a print in the queue on a Bambu Labs printer while actively printing on a Formlabs printer.
